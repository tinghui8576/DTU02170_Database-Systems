...
#2.1 Get all the departments of the university.
#SELECT * FROM Department;
#2.2 Get the name and building of all departments of the university.
#SELECT DeptName, Building FROM Department;
#2.3 Get all departments with a budget greater than or equal to 100000.
#SELECT * FROM Department WHERE Budget >= 100000;
#2.4 Get all departments with a budget greater than or equal to 70000 and that is located in the Taylor building
#SELECT * FROM Department WHERE Budget >= 70000 AND Building = "Taylor";
#2.5 Get the instructors (id and name) that work in a department with a budget >= 100000.
...
#2.6 Increase the salary of each instructor in the Comp. Sci. department by 10%.
#UPDATE Instructor SET Salary = Salary * 1.10 WHERE DeptName = 'Comp. Sci.';
#SELECT * FROM Instructor;
#2.7 Delete all courses with credits less than or equal to 3.
# safe mode disable
...
# 2.8 Insert every student whose TotCreds attribute is greater than 100 as an instructor in the 
# same department, with a salary of 10000.
#INSERT INTO Instructor SELECT StudID, StudName, DeptName, 10000 
#FROM Student WHERE TotCredits >100;
# 2.10 to provide a conversion from letter grades in the Takes table to numeric scores; 
# For example an “A” grade could be specified to corresponds to 4 points, an “A-“ to 3.7 points, 
# a “B+” to 3.3 points and so on.
#CREATE TABLE GradePoints ( Grade VARCHAR(2) PRIMARY KEY, Points DECIMAL(3,1));
